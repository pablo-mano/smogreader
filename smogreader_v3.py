...
while True:
	...
	filecontent = str(timestamp) + ';' + str(pm25) + ';' + str(pm10) + ';' + str(temperature) + ';' + str(pressure) + ';' + str(humidity) + '\n'
        ...
        opensmogjson = '[{ "timestamp": ' + str(int(unixtimestamp)) + ',"readings": {"PM2_5": ' + str(pm25) +',"PM10": ' + str(pm10) +',"TEMP": ' + str(temperature) +',"HUM": ' + str(humidity) +',"PRES": ' + str(pressure) +'}}]'
	
	f = open('smogreader_sdl_bme.txt','a')
	f.write(filecontent)
	f.close() 
	r = requests.get('http://dweet.io/dweet/for/{your-dweet-id}?key={your-dweet-key}&'+dweetcontent)
